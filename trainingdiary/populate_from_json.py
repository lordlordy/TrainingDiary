...
class DataWarehouseManager:
    ...
    def __init__(self, json_file_name, db_name):
    ...
    def populate_from_json(self, print_progress=False):

        days = self.__data['days']
        self.populate_days(days, print_progress=print_progress)


    def populate_days(self, days, print_progress=False):
    ...
    def populate_kg_from_json(self, print_progress=False):
    ...
    def populate_lbs_from_json(self, print_progress=False):
        f = open('TrainingDiary.json')
        data = json.load(f)
    ...
    def populate_fat_percent_from_json(self, print_progress=False):

        f = open('TrainingDiary.json')
        data = json.load(f)

        if 'fatPercentage' in data:
            data_value_array = [(parser.parse(d['iso8601DateString']).date(), float(d['value'])) for d in data['fatPercentage']]
            self.__populate_and_interpolate_values('fat_percentage', data_value_array, print_progress=print_progress)


    def populate_hr_from_json(self, print_progress=False):

        f = open('TrainingDiary.json')
        data = json.load(f)

        if 'restingHR' in data:
            data_value_array = [(parser.parse(d['iso8601DateString']).date(), int(d['value'])) for d in data['restingHR']]
            self.__populate_and_interpolate_values('resting_hr', data_value_array, print_progress=print_progress)


    def populate_sdnn_from_json(self, print_progress=False):

        f = open('TrainingDiary.json')
        data = json.load(f)

        if 'restingSDNN' in data:
            data_value_array = [(parser.parse(d['iso8601DateString']).date(), float(d['value'])) for d in data['restingSDNN']]
            self.__populate_and_interpolate_values('sdnn', data_value_array, print_progress=print_progress)


    def populate_rmssd_from_json(self, print_progress=False):

        f = open('TrainingDiary.json')
        data = json.load(f)

        if 'restingRMSSD' in data:
            data_value_array = [(parser.parse(d['iso8601DateString']).date(), float(d['value'])) for d in data['restingRMSSD']]
            self.__populate_and_interpolate_values('rmssd', data_value_array, print_progress=print_progress)


    def __populate_and_interpolate_values(self, column_name, date_value_list, min_date=None, max_date=None, print_progress=False):
        ...
        dates_array = [i[0] for i in date_value_list]
        values_array = [i[1] for i in date_value_list]
    ...
    def calculate_all_strain(self, from_date=None, print_progress=False):
        ...
        for table_name in t_list:
            ...
            df['rpe_monotony'] = df['rpe_tss'].rolling(STRAIN_DAYS, min_periods=min_periods).mean() / df['rpe_tss_stdev']
            ...
            for index, row in df.iterrows():
                ...
                if process:
                    ...
                    conn.cursor().execute(sql_str)
                    if print_progress:
                        print(f'{t_count/len(t_list):.2%} {datetime.datetime.now() - st} {table_name}', '               ', end='\r')
    ...
    def __save_workout(self, conn, d_date, d_type, d_values, workout, keys):
        ...
        self.__execute_day_sql(conn, d_date, d_type, col_names, d_values, activity=a, activity_type=at, equipment_name=e_name)
    ...
    def __execute_day_sql(self, conn, d_date, d_type, col_names, values, activity='All', activity_type='All', equipment_name='All', table_name=None):
...
if __name__ == '__main__':
    ...
    dwm = DataWarehouseManager(json_file_name='TrainingDiary.json', db_name='training_data_warehouse.sqlite3')

    if all or day:
        print('Basic day info...')
        dwm.populate_from_json(print_progress)

    if all or kg:
        print('KG...')
        dwm.populate_kg_from_json(print_progress)

    if all or lbs:
        print('LBS...')
        dwm.populate_lbs_from_json(print_progress)

    if all or fat:
        print('Fat%...')
        dwm.populate_fat_percent_from_json(print_progress)

    if all or hr:
        print('HR...')
        dwm.populate_hr_from_json(print_progress)

    if all or sdnn:
        print('SDNN...')
        dwm.populate_sdnn_from_json(print_progress)

    if all or rmssd:
        print('RMSSD...')
        dwm.populate_rmssd_from_json(print_progress)
